...
from . import label_view, table_view
from .core import Window, ScrollTextWindow
...
class StructureListWindow(Window):
    ...
    def __init__(self, viewer, quiet=False, lazy_load=False):
        ...
        self._add_menu(quiet, lazy_load)
    ...
    def open_label(self, filename=None, from_existing_structures=None):
        ...
        # Determine whether numpy was imported
        use_numpy = True
        if np is None:
            use_numpy = False
        ...
        try:
            ...
            if filename is not None:
                ...
                self._structure_list = pds4_read(filename, quiet=self.menu_option('quiet'), lazy_load=True,
                                                 decode_strings=False, use_numpy=use_numpy)
            ...
            self._set_title("{0} - {1}".format(self._widget.title(), title))
            ...
            error = log + '\n' + trace
            ScrollTextWindow(self._viewer, 'An Error Occurred!', error)
            logger.exception(e)
    ...
    def _reify_structure(self, structure, size_check=True):
        ...
        if exception_occurred:
            ScrollTextWindow(self._viewer, 'An Error Occurred!', log)
    ...
    def _draw_summary(self):
        ...
        view_menu.add_command(label='Read-In Log', command=lambda:
            ScrollTextWindow(self._viewer, 'Label/Data Read-in Log', self._structure_list.read_in_log))

        # Draw summary for structures found
        if len(self._structure_list) > 0:
            self._draw_structure_summary()

        # Draw summary that shows only the full label if no data structures are found
        else:
    ...
    def _open_plot(self, structure_idx):
        ...
        # Import plot view; this module requires NumPy and MPL, so we import it
        # here as opposed to at the top.
        ...
        if _is_plottable(structure):
            ...
            raise TypeError('Cannot show plot non-plottable structure with type ' + structure.type)

    # Opens an image view for a structure
    def _open_image(self, structure_idx):

        # Import image view; this module requires NumPy and MPL, so we import it
        # here as opposed to at the top.
    ...
    def _open_file_box(self, new_window=True):
        ...
        filename = askopenfilename(title='Open Label',
                                   parent=self._widget,
                                   filetypes=filetypes,
                                   initialdir=self._viewer.last_open_dir)
    ...
    def _add_menu(self, quiet, lazy_load):
    ...
    def reset(self):
        ...
        if self._label_open:
            ...
            self._set_title(self._widget.title().split('-')[0].strip())
            self._menu.delete(self._menu.index('last'))
...
def _is_tabular(structure):
    ...
    if structure.is_table() or structure.is_array():
        return True
