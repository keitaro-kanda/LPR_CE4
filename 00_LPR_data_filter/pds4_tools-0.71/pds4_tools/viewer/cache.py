...
def _get_cache_version():
    ...
    try:
        ...
        logger.warn('Unable to read version from cache file: {0}'.format(str(e)))
...
def _write_cache_version():
    ...
    try:
        ...
        logger.warn('Unable to write version to cache file: {0}'.format(str(e)))
        return False
...
def _set_mpl_cache():
    ...
    if _get_cache_version() != _get_current_version():
        ...
        logger.warn('Copying mpl-data to enable permanent MPL cache. This should only run once,'
        ...
        try:
            ...
            error_occurred = True
            logger.warn('Unable to set MPL datapath. MPL cache may not work. '
