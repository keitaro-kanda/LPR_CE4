...
class TabularViewWindow(DataViewWindow):
    ...
    def load_table(self, table_structure):
        ...
        self._set_title("{0} - Table '{1}'".format(self._widget.title(), table_structure.id))

        # Set necessary instance variables for this DataViewWindow
        self.data = table_structure.data
        ...
        # Handle case where data point is a single scalar value (this is most of the time)
            ...
            if self.menu_option('display_data_as_formatted'):
                ...
                try:
                    ...
                    self._issue_warning("Unable to format value '{0}' into field_format '{1}'; displaying "
                                        "unformatted value".format(data_point, meta_data['format']), show=False)
...
def array_like_to_table(array_like, name, data_type, format=None, full_label=None, array_label=None):
    ...
    # Create an incomplete TableStructure
    table_structure = TableStructure(structure_label=array_label, full_label=full_label)
    table_structure.id = name
    ...
    list_depth_func = lambda L: (is_array_like(L[0]) and list_depth_func(L[0])+1 or 1)
    array_depth = list_depth_func(array_like)
    ...
    if (array_depth == 1) or (array_depth == 3):
        table_structure.add_field(array_like, Meta_Field(meta_dict), use_numpy)
    ...
    # For arrays other than 1D and 3D
        ...
        if use_numpy:
            array_like = np.asanyarray(array_like).swapaxes(0, 1)

        else:
            array_like = map(list, zip(*array_like))
        ...
        for field_num, field_data in enumerate(array_like):
            ...
            meta_data = Meta_Field(meta_dict)
            meta_data['name'] = '{0} : {1}'.format(field_num, meta_data['name'])

            table_structure.add_field(field_data, meta_data, use_numpy)

    return table_structure


# Transforms an Array Structure to a Table Structure
def array_structure_to_table(array_structure):
