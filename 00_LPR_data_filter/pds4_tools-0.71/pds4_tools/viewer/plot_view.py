...
class PlotViewWindow(DataViewWindow):
    ...
    # connecting the data points; set `points` to have markers shown on data points.
    def load_table(self, table_structure, axis_selections, lines=True, points=False):
        ...
        if self._data_open:
            ...
            self._draw_series(series_idx, lines=lines, points=points)
            return
        ...
        self._set_title("{0} - Plot from '{1}'".format(self._widget.title(), table_structure.id))
    ...
    def get_tick_labels(self, axis, which='both', mpl_text=False, if_visible=False):
        ...
        if if_visible and (not self.is_tick_labels_shown(axis)):
            tick_labels = None
            ...
            ax = self._plot.axes
            axis = ax.xaxis if axis == 'x' else ax.yaxis
            mpl_labels = axis.get_ticklabels(which=which)
    ...
    def get_error_bar_options(self, series, which):
        ...
        # when doing `set_linestyle`. The returned value is converted via the `dash_dict`, which we resolve
        # back to the initially passed in value.
    ...
    def set_grid_options(self, which='both', style=None, color=None, show=None):
        ...
        if which in ('major', 'minor', 'both'):
            ...
            # Set grid style
            if style is not None:
                self._plot.axes.grid(which=which, linestyle=style)

            # Set grid color
            if color is not None:
                self._plot.axes.grid(which=which, color=color)

            # Setting the grid style and color above automatically displays grid even if it was not suppose
            # to be displayed. We disable it again here, while preserving the linestyle and color changes.
            is_major_shown = self.menu_option('show_major_grid')
            is_minor_shown = self.menu_option('show_minor_grid')

            if (not is_major_shown) and (not is_minor_shown):
                self._plot.axes.grid(b=False, which=which)
    ...
    def _draw_series(self, series, lines=None, points=None):
        ...
        _series = self.series[series]
        plot_line, error_caps, error_lines = self._plot.errorbar(_series.data('x'),
                                                                 _series.data('y'),
                                                                 xerr=_series.data('x_err'),
                                                                 yerr=_series.data('y_err'),
                                                                 zorder=500)
        ...
        self.set_error_bar_options(series, style='dotted', color='#3535ff')
    ...
    def _update_axis_limits(self, *args):
        ...
        # Automatic limits. Usually uses axis limits which are somewhat bigger than data limits.
            ...
            # try:
            #     axis.set_xmargin(mpl.rcParams['axes.xmargin'])
            #     axis.set_ymargin(mpl.rcParams['axes.ymargin'])
            #
            # except:
            #     pass
    ...
    def _save_file_box(self):
        ...
        initial_dir = self._viewer.last_open_dir
        ...
        self._viewer.last_open_dir = os.path.dirname(filename)
...
class PlotOptionsWindow(Window):
    ...
    def __init__(self, viewer, master, plot_view_window, initial_display=None):
        ...
        # Set the title
        self._set_title('{0} - Plot Options'.format(self._widget.title()))
        ...
        self._set_window_dimensions(785, 310)
        self._center_window()
...
class PlotColumnsWindow(Window):
    ...
    def __init__(self, viewer, table_structure):
        ...
        self._set_title("{0} - Select Plot Columns from Table '{1}'".
                        format(self._widget.title(), table_structure.id))
        ...
        # Controls whether lines and points checkbuttons are checked
        # (and subsequently which type will be drawn for the plot)
    ...
    def draw_content(self):
        ...
        for i, field in enumerate(self._structure.fields):
            ...
            is_numeric_array = np.issubdtype(field.dtype, np.integer) or \
                               np.issubdtype(field.dtype, float) or \
                               (field.dtype == np.dtype(object) and isinstance(field[0], six.integer_types))
        ...
        # add_plot = Checkbutton(middle_box, text='Add curve to the current graph')
...
class _Series(object):
    ...
    def meta_data(self, axis):
        ...
        if axis_selection is None:
            ...
            meta_data = self.data(axis).meta_data
...
def open_plot(viewer, table_structure, axis_selections, lines=True, points=False):
    ...
    plot_window.load_table(table_structure, axis_selections, lines=lines, points=points)
